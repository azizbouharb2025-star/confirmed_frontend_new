...
function AIScoreSection({ order }: { order: Order }) {
  ...
  return (
    <div
      className="py-5 border-b border-gray-200 dark:border-slate-700"
        ...
        {risk && (
          <div className="mt-4 h-2 overflow-hidden rounded-full bg-gray-200 dark:bg-slate-700">
            <div
              ...
              style={{
                width: `${Math.max(0, Math.min(100, order.aiScore))}%`,
              }}
            />
          </div>
        )}
          ...
          <div className="mt-4 border-t border-gray-200 pt-4 dark:border-slate-700">
            <div className="flex items-center justify-between gap-4 mb-3">
              ...
              <div className="text-right text-xs text-gray-500 dark:text-slate-400">
                Base {order.aiScoreDetails.baseScore}
                {' → '}
                Final {order.aiScoreDetails.finalScore}
              </div>
            </div>

            <div className="space-y-2">
              {order.aiScoreDetails.factors.map((factor, index) => (
                <div
                  key={factor._id || `${factor.key}-${index}`}
                  className="flex items-center justify-between gap-4 rounded-lg border border-gray-200 bg-white px-3 py-3 dark:border-slate-700 dark:bg-slate-800"
                >
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-gray-900 dark:text-white">
                      {factor.label}
                    </p>
                    <p className="mt-0.5 text-xs text-gray-500 dark:text-slate-400 break-words">
                      Valeur : {formatFactorValue(factor.value)}
                    </p>
                  </div>
              ...
              >
                <Dialog.Panel className="pointer-events-auto w-screen max-w-xl sm:max-w-2xl">
                  <div className="flex h-full flex-col bg-white dark:bg-slate-800 shadow-xl">
                    {/* Header */}
                    <div className="px-5 py-5 sm:px-7 border-b border-gray-200 dark:border-slate-700">
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          <Dialog.Title className="text-xl font-bold text-gray-900 dark:text-white break-all">
                    ...
                    {/* Content */}
                    <div className="flex-1 overflow-y-auto px-5 sm:px-7" data-testid="order-detail-content">
