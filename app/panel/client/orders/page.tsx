  ...
  const handleManualSubmit = async () => {
    ...
    try {
      ...
      const payload: Parameters<typeof orderService.createOrder>[0] = {
        ...
        totalAmount,
        ...((manualForm.estimatedDate ||
             manualForm.trackingNumber ||
             manualForm.carrier ||
             manualForm.secondaryPhone ||
             manualForm.comment ||
             manualForm.weight > 0 ||
             manualForm.colissimoType) && {
          deliveryInfo: {
            ...(manualForm.estimatedDate && {
              estimatedDate: new Date(manualForm.estimatedDate).toISOString()
            }),
            ...(manualForm.trackingNumber && {
            ...
            ...(manualForm.secondaryPhone && {
              secondaryPhone: manualForm.secondaryPhone
            }),
            ...(manualForm.colissimoType && {
              packageCount: manualForm.packageCount,
              colissimoType: manualForm.colissimoType as
                'VO' | 'VM' | 'GV' | 'EXP' | 'FIX' | 'ONP' | 'BLK' | 'SMD'
            }),
            ...(manualForm.comment && {
                ...
                <div className={editingOrder ? 'hidden' : ''}>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <input
                      ...
                      onChange={e => setManualForm(p => ({ ...p, secondaryPhone: e.target.value }))}
                      className="w-full px-4 py-2.5 rounded-lg dark:bg-slate-800 bg-gray-50 border-2 dark:border-slate-600 border-gray-300 focus:border-blue-500 dark:text-white text-gray-900 outline-none transition-all"
                    />
                    ...
                    <input
                      ...
                      value={manualForm.packageCount}
                      onChange={e => setManualForm(p => ({
                        ...p,
                        packageCount: Math.max(1, parseInt(e.target.value) || 1)
                      }))}
                      className="w-full px-4 py-2.5 rounded-lg dark:bg-slate-800 bg-gray-50 border-2 dark:border-slate-600 border-gray-300 focus:border-blue-500 dark:text-white text-gray-900 outline-none transition-all"
                    />
                    ...
                    <select
                      value={manualForm.colissimoType}
                      onChange={e => setManualForm(p => ({
                        ...p,
                        colissimoType: e.target.value
                      }))}
                      className="w-full px-4 py-2.5 rounded-lg dark:bg-slate-800 bg-gray-50 border-2 dark:border-slate-600 border-gray-300 focus:border-blue-500 dark:text-white text-gray-900 outline-none transition-all"
                    >
                      <option value="">Type Colissimo</option>
                    ...
                    <textarea
                      placeholder="Commentaire Colissimo"
                      value={manualForm.comment}
                      onChange={e => setManualForm(p => ({ ...p, comment: e.target.value }))}
                      rows={2}
                      className="w-full px-4 py-2.5 rounded-lg dark:bg-slate-800 bg-gray-50 border-2 dark:border-slate-600 border-gray-300 focus:border-blue-500 dark:text-white text-gray-900 outline-none transition-all col-span-2 resize-none"
                    />
                  </div>
                </div>
              </div>
